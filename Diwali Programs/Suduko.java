...
public class Suduko{
	...
	public static void find(String s[][],int i, int j,int m){
		...
		for(k=1,l=49;k<=m && l<=57;k++,l++){
			...
			for(a=0;a<m;a++){
				for(b=0;b<m;b++){
					if(a==i || b==j){
						if(s[a][b].charAt(0)==l && box(s,l,a,b)){
							flag=1;
							//break;
						}
					}
			...
			if(flag==0){
				...
				for(x=1,y=49;x<=m && y<=57;x++,y++){
					flag1=0;
					for(z=0;z<m;z++){
						if(s[i][z].charAt(0)==y && box(s,y,i,z))
							flag1=1;
					}
					if(flag1==0){
	...
	public static boolean box(String arr[][],int num,int r,int c){
		int rindex=(r/3)*3;
		int cindex=(c/3)*3;
		for(int i=0;i<3;i++)
			for(int j=0;j<3;j++)
				if(arr[rindex+i][cindex+j].charAt(0)==num)
					return true;
		return false;
	}
}
